...
// Define the KV interface for type safety
  ...
  implements SessionManager<V>
{
  ...
  constructor(kvNamespace: CloudflareKV, options: KvStorageOptions = {}) {
    ...
    this.defaultTtl = options.defaultTtl || 3600; // Default 1 hour TTL
  ...
  async setSessionItem(
    ...
    try {
      ...
      if (typeof itemValue === "string") {
        ...
        // Store non-string values as JSON with index 0
      ...
      // Verify write succeeded if consistency checks enabled
  ...
  async getSessionItem(itemKey: V | StorageKeys): Promise<unknown | null> {
    ...
    // Try with retries for eventual consistency
  ...
  private async _getSessionItemOnce(itemKey: V | StorageKeys): Promise<unknown | null> {
    try {
      ...
      // Reconstruct the value from all chunks
      let itemValue = "";
      let index = 0;
  ...
  async setItems(items: Record<string, unknown>): Promise<void> {
    if (this.enableConsistencyChecks) {
      // Sequential writes for consistency verification
  ...
  async removeSessionItem(itemKey: V | StorageKeys): Promise<void> {
    try {
      // Find all chunks for this key by checking sequentially
      const keysToDelete: string[] = [];
      let index = 0;
      
      // Check for chunks until we find a missing one
      while (true) {
      ...
      // Delete all found chunks in parallel
      await Promise.all(
